#!/usr/bin/env python3
"""Idempotently add the scoped WLOC branch to an existing mosdns config."""
...
DOMAIN_PLUGIN = """  - tag: geosite_wloc
    type: domain_set
...
def migrate_text(text):
    ...
    if not all(marker in migrated for marker in markers):
        raise ValueError("mosdns WLOC migration validation failed")
