...
assert 'edit(chat, mid, msg if ok else ("❌ " + msg), OPS_BACK)' in bot and "set_tfo" in bot, (
...
assert "delete_message" in bot and "正在处理上一项操作" in bot, (
    "must delete exit credential messages and busy-callback like 5GPN-X"
)
assert "_CB_EXECUTOR" in bot or "pdg-cb" in bot, "callback answer should use a dedicated pool"
