...
| 20443 | tcp | 全网·仅启用 Relay 时 | iOS 17+ Apple Network Relay HTTP/2 入口；由 `pdg-relay` 动态 nft 表放行 |
...
全量 iOS Relay 默认关闭。需要时运行 `sudo pdg relay enable`，并确保云厂商安全组允许
`20443/tcp`；详情与回退步骤见 [RELAY.md](RELAY.md)。
...
- 写入: `/etc/mosdns/rules/geosite_cn.txt`(旧 DoT DNS 直连) + `ChinaMax.list`(原始)；同时在服务器生成 `/etc/mihomo/rs/__pdg_china_{domain,ip}.mrs`，供全量 Relay 国内直出。实际规则文件不存放在 GitHub。
