...
在 `🧩 MITM 插件管理` 中，删除以单个 Kelee/Loon 插件 URL 为单位；点 `➕ 添加插件 URL` 后发送一个 HTTPS 插件地址即可。完整插件列表每页显示 10 个。首次添加会固定模块正文 SHA256。每日检查发现正文变化时，旧的 last-known-good 版本继续运行；去广告首页会显示 `⬆️ 待批准更新` 直达页，展示主机及有限的新增/删除规则摘要。点“批准并应用”后会再次核对完整 SHA；确认仅绑定当前聊天管理员、当前来源配置和候选内容，10 分钟内只能使用一次。删除的插件不会在升级时自动恢复。
