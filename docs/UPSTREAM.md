...
[Xiuyixx/5GPN-X](https://github.com/Xiuyixx/5GPN-X) 是 dnsdist+sniproxy 路线。可借鉴 UX, 不能整仓合:

| 5GPN-X | 本仓对应 |
|--|--|
| tgbot 异步 + 删出口凭据消息 | `pdg-bot` `run_bg` / `delete_message` |
| 853 公网 DoT | nft `tcp dport 853 accept` |
| wa-shim 无 SNI | mosdns `geosite_wa` 返回真实 A + TPROXY |
